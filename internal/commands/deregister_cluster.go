...
func DeregisterCluster(ctx context.Context, args []string, logger logr.Logger) error {
	doc := `Usage:
	sveltosctl deregister <command> [<args>...]

	cluster       Removes a non CAPI cluster that was previously registered with Sveltos.

Options:
	-h --help      Show this screen.

Description:
	See 'sveltosctl deregister <command> --help' to read about a specific subcommand.
  `
	...
	switch command {
	case clusterCommand:
		return onboard.DeregisterCluster(ctx, arguments, logger)
	default:
