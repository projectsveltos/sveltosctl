...
var _ = Describe("Cluster utils", func() {
	...
	It("isClusterPaused returns true when Spec.Paused is set to true", func() {
		...
		a := utils.GetAccessInstance()

		paused, err := a.IsClusterPaused(context.TODO(), cluster.Namespace,
			cluster.Name, libsveltosv1alpha1.ClusterTypeCapi)
		Expect(err).To(BeNil())
		Expect(paused).To(BeTrue())

		paused, err = a.IsClusterPaused(context.TODO(), sveltosCluster.Namespace,
	...
	It("getSecretData returns kubeconfig data", func() {
		...
		data, err := a.GetSecretData(context.TODO(), cluster.Namespace, cluster.Name, "",
			libsveltosv1alpha1.ClusterTypeCapi, klogr.New())
		Expect(err).To(BeNil())
		Expect(data).To(Equal(randomData))

		data, err = a.GetSecretData(context.TODO(), sveltosCluster.Namespace, sveltosCluster.Name, "",
