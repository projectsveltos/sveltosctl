...
func (a *k8sAccess) GetCluster(ctx context.Context,
	...
	if clusterType == libsveltosv1alpha1.ClusterTypeSveltos {
		return getSveltosCluster(ctx, a.client, clusterNamespace, clusterName)
	}
	return getCAPICluster(ctx, a.client, clusterNamespace, clusterName)
}
...
func (a *k8sAccess) IsClusterPaused(ctx context.Context,
	clusterNamespace, clusterName string, clusterType libsveltosv1alpha1.ClusterType) (bool, error) {

	if clusterType == libsveltosv1alpha1.ClusterTypeSveltos {
		return isSveltosClusterPaused(ctx, a.client, clusterNamespace, clusterName)
	}
	return isCAPIClusterPaused(ctx, a.client, clusterNamespace, clusterName)
}
...
func (a *k8sAccess) GetSecretData(ctx context.Context, clusterNamespace, clusterName, admin string,
	clusterType libsveltosv1alpha1.ClusterType, logger logr.Logger) ([]byte, error) {

	if admin != "" && admin != clusterAdmin {
		return roles.GetKubeconfig(ctx, a.client, clusterNamespace, clusterName, admin, clusterType)
	}

	if clusterType == libsveltosv1alpha1.ClusterTypeSveltos {
		return clusterproxy.GetSveltosSecretData(ctx, logger, a.client, clusterNamespace, clusterName)
	}
	return clusterproxy.GetCAPISecretData(ctx, logger, a.client, clusterNamespace, clusterName)
}

func (a *k8sAccess) GetKubernetesRestConfig(ctx context.Context, clusterNamespace, clusterName, admin string,
	clusterType libsveltosv1alpha1.ClusterType, logger logr.Logger) (*rest.Config, error) {

	if admin != "" && admin != clusterAdmin {
		return getKubernetesRestConfigForAdmin(ctx, a.client, clusterNamespace, clusterName, admin, clusterType, logger)
	}

	if clusterType == libsveltosv1alpha1.ClusterTypeSveltos {
		return clusterproxy.GetSveltosKubernetesRestConfig(ctx, logger, a.client, clusterNamespace, clusterName)
	}
	return clusterproxy.GetCAPIKubernetesRestConfig(ctx, logger, a.client, clusterNamespace, clusterName)
}

func (a *k8sAccess) GetKubernetesClient(ctx context.Context, clusterNamespace, clusterName, admin string,
	clusterType libsveltosv1alpha1.ClusterType, logger logr.Logger) (client.Client, error) {

	if admin != "" && admin != clusterAdmin {
		return getKubernetesClientForAdmin(ctx, a.client, clusterNamespace, clusterName, admin, clusterType, logger)
	}

	if clusterType == libsveltosv1alpha1.ClusterTypeSveltos {
		return clusterproxy.GetSveltosKubernetesClient(ctx, logger, a.client, a.scheme, clusterNamespace, clusterName)
	}
	return clusterproxy.GetCAPIKubernetesClient(ctx, logger, a.client, a.scheme, clusterNamespace, clusterName)
}

func (a *k8sAccess) GetClusterType(cluster *corev1.ObjectReference) libsveltosv1alpha1.ClusterType {
	// TODO: remove this
